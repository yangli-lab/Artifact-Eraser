...
def main():
	...
	previous_opts = previous_train_ckpt['opts']
	opts_dict = vars(opts)
	for k,v in previous_opts.items():
		if k in opts_dict.items():
			previous_opts[k] = opts_dict[k]
		opts = Namespace(**opts_dict)
	train = Train(opts, previous_train_ckpt)
	train.train()
